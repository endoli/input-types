...
impl PrimaryPointerState {
    ...
    pub fn just_pressed(&self, button: PointerButton) -> bool {
        self.events
            .iter()
            .any(|e| matches!(e, &PointerEvent::Down(PointerButtonEvent { button: b, .. }) if b == Some(button)))
    }

    pub fn just_released(&self, button: PointerButton) -> bool {
        self.events
            .iter()
            .any(|e| matches!(e, &PointerEvent::Up(PointerButtonEvent { button: b, .. }) if b == Some(button)))
    }
